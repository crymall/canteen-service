...
router.get('/popular', authenticateToken, authorizePermission('read:canteen'), async function(req, res, next) {
  try {
    const limit = Math.min(parseInt(req.query.limit) || 50, 50);
    const offset = parseInt(req.query.offset) || 0;
    const query = `
...
/* PUT update recipe. */
router.put('/:id', authenticateToken, authorizePermission('write:canteen'), async function(req, res, next) {
  try {
    const { id } = req.params;
    const { title, description, instructions, prep_time_minutes, cook_time_minutes, servings } = req.body;
    const result = await pool.query(
      ...
      [title, description, instructions, prep_time_minutes, cook_time_minutes, servings, id, req.user.id]
    );
    if (result.rows.length === 0) {
...
router.post('/', authenticateToken, authorizePermission('write:canteen'), async function(req, res, next) {
  try {
    ...
    const result = await pool.query(
      ...
      [req.user.id, title, description, instructions, prep_time_minutes, cook_time_minutes, servings]
    );
...
router.post('/:id/ingredients', authenticateToken, authorizePermission('write:canteen'), async function(req, res, next) {
  try {
    const { id } = req.params;
...
router.post('/:id/tags', authenticateToken, authorizePermission('write:canteen'), async function(req, res, next) {
  try {
    const { id } = req.params;
...
router.post('/:id/likes', authenticateToken, authorizePermission('write:canteen'), async function(req, res, next) {
  try {
    const { id } = req.params;
    const result = await pool.query('INSERT INTO recipe_likes (user_id, recipe_id) VALUES ($1, $2) RETURNING *', [req.user.id, id]);
    res.status(201).json(result.rows[0]);
  } catch (err) {
    next(err);
  }
...
/* DELETE recipe like. */
router.delete('/:id/likes', authenticateToken, authorizePermission('write:canteen'), async function(req, res, next) {
  try {
    const { id } = req.params;
    const result = await pool.query('DELETE FROM recipe_likes WHERE recipe_id = $1 AND user_id = $2 RETURNING *', [id, req.user.id]);
    if (result.rows.length === 0) {
...
/* DELETE recipe tag. */
router.delete('/:id/tags/:tagId', authenticateToken, authorizePermission('write:canteen'), async function(req, res, next) {
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Tag not found on recipe or unauthorized' });
    }
...
router.delete('/:id/ingredients/:ingredientId', authenticateToken, authorizePermission('write:canteen'), async function(req, res, next) {
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Ingredient not found on recipe or unauthorized' });
    }
